...
module.exports = Class.create({
	...
	startup: function (callback) {
		...
		this.web.addURIHandler(/^\/db$/i, 'Reports', __dirname + "/../htdocs/custom/dashboard.html");
		this.web.addURIHandler(/^\/console$/i, 'Console', __dirname + "/../htdocs/custom/console.html");
	...
	checkmanagerEligibility: function (callback) {
		...
		this.storage.listGet('global/servers', 0, 0, function (err, servers) {
			...
			if (self.server.config.get('manager')) {
				self.logDebug(4, "Nominating self (" + self.server.hostname + ") as manager based on `manager` config setting");
				self.gomanager();
				return;
			}
